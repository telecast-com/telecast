...
function getNowInfo(schedule, now) {
  ...
  const elapsedSeconds = Math.max(0, (effectiveNow - startMin) * 60 + now.getSeconds());
  return { currentIdx: idx, nextIdx, current: schedule[idx], next: schedule[nextIdx], progress, minutesLeft, elapsedSeconds };
}
...
function ChannelScreen({ channel, big, t, videoUrl, startOffsetSeconds }) {
  ...
  useEffect(() => {
    ...
    function onLoaded() {
      try {
        const offset = (startOffsetSeconds || 0) % (v.duration || 1);
  ...
  return (
    <div className={cx("relative overflow-hidden rounded-xl bg-gradient-to-br", channel.grad[0], channel.grad[1],
      ...
      {videoUrl ? (
        <video ref={videoRef} src={videoUrl} muted autoPlay playsInline loop
...
const GRAD_BY_CATEGORY = {
  movies: ["from-red-900", "to-zinc-950"], music: ["from-fuchsia-900", "to-zinc-950"],
  sports: ["from-emerald-900", "to-zinc-950"], education: ["from-amber-900", "to-zinc-950"],
  gaming: ["from-violet-900", "to-zinc-950"], news: ["from-sky-900", "to-zinc-950"],
  entertainment: ["from-rose-900", "to-zinc-950"],
};

async function fetchDbChannels() {
  ...
  return channels.map((row, i) => ({
    ...
    tagline: row.tagline || "A new channel on TeleCast.",
    followers: row.followers_count || 0,
    viewersBase: Math.floor(Math.random() * 30) + 5,
    owner: "TeleCast Creator",
    desc: row.description || row.tagline || `${row.name} is a new channel on TeleCast.`,
    grad: GRAD_BY_CATEGORY[row.category] || GRAD_BY_CATEGORY.entertainment,
    isNew: true,
    isLive: true,
    isDb: true,
    ownerId: row.owner_id,
    schedule: (row.schedule_items && row.schedule_items.length
      ? [...row.schedule_items].sort((a, b) => a.sort_order - b.sort_order).map((s) => ({ id: s.id, time: s.slot_time, title: s.title, videoUrl: s.video_url || null }))
      : buildSchedule(row.category)),
  }));
}

async function createDbChannel({ name, category, tagline, ownerId }) {
  const { data: channel, error } = await supabase
    .from("channels")
    .insert({ owner_id: ownerId, name, category, tagline })
    .select()
    .single();
  if (error) throw error;
  const slots = buildSchedule(category);
  const items = slots.map((s, i) => ({ channel_id: channel.id, slot_time: s.time, title: s.title, sort_order: i }));
  await supabase.from("schedule_items").insert(items);
  return channel;
}

async function addScheduleItem(channelId, time, title, sortOrder) {
  const { error } = await supabase
    .from("schedule_items")
    .insert({ channel_id: channelId, slot_time: time, title, sort_order: sortOrder });
  if (error) throw error;
}
...
async function adminDeleteChannel(channelId) {
  const { error } = await supabase.from("channels").delete().eq("id", channelId);
  if (error) throw error;
}

async function fetchUserFollows(userId) {
  const { data, error } = await supabase.from("follows").select("channel_id").eq("user_id", userId);
  if (error || !data) return [];
  return data.map((r) => r.channel_id);
}

async function dbFollow(userId, channelId) {
  await supabase.from("follows").insert({ user_id: userId, channel_id: channelId });
}
async function dbUnfollow(userId, channelId) {
  await supabase.from("follows").delete().eq("user_id", userId).eq("channel_id", channelId);
}

async function uploadScheduleVideo(scheduleItemId, channelId, file) {
  const ext = file.name.split(".").pop();
  const path = `${channelId}/${scheduleItemId}-${Date.now()}.${ext}`;
  const { error: uploadError } = await supabase.storage.from("channel-videos").upload(path, file, { upsert: true });
  if (uploadError) throw uploadError;
  const { data: pub } = supabase.storage.from("channel-videos").getPublicUrl(path);
  const { error: updateError } = await supabase.from("schedule_items").update({ video_url: pub.publicUrl }).eq("id", scheduleItemId);
          ...
          <div className={cx("rounded-2xl border overflow-hidden grid md:grid-cols-2", t.border, t.surface)}>
            ...
            <div className="p-4 sm:p-6 flex items-center">
              <ChannelScreen big channel={{ ...featured, currentTitle: getNowInfo(featured.schedule, now).current.title }} t={t}
                videoUrl={getNowInfo(featured.schedule, now).current.videoUrl} startOffsetSeconds={getNowInfo(featured.schedule, now).elapsedSeconds} />
            </div>
          </div>
        </section>
      )}
  ...
  return (
    <div>
      <ChannelScreen big channel={{ ...channel, currentTitle: info.current.title }} t={t}
        videoUrl={info.current.videoUrl} startOffsetSeconds={info.elapsedSeconds} />
...
function OwnerDashboard({ t, isDark, ownedChannels, now, addChannel, user, onOpenAuth, onRefreshChannels }) {
  ...
  async function handleAddSlot(e) {
    e.preventDefault();
    if (!newSlotTime || !newSlotTitle.trim() || !selected?.isDb) return;
    setAddingSlot(true);
    try {
      await addScheduleItem(selected.id, newSlotTime, newSlotTitle.trim(), schedule.length);
      await onRefreshChannels();
      setNewSlotTime("");
      setNewSlotTitle("");
    } catch (err) {
      alert("Dastur qo'shishda xatolik: " + (err.message || JSON.stringify(err)));
    } finally {
      setAddingSlot(false);
    }
  }
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setCreateError((err && (err.message || err.details || err.hint)) || JSON.stringify(err) || "Noma'lum xatolik");
    } finally {
      setCreating(false);
    }
  }
    ...
    <div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} className={cx("rounded-xl border p-5 mb-8 grid sm:grid-cols-3 gap-4", t.border, t.surface)}>
          {!user && (
            <div className="sm:col-span-3 flex items-center justify-between text-xs rounded-lg border border-red-500/40 bg-red-500/10 px-3 py-2 text-red-500">
              Kanal yaratish uchun avval tizimga kiring.
              <button type="button" onClick={onOpenAuth} className="font-semibold underline">Kirish</button>
            </div>
          )}
          ...
          <div className="sm:col-span-3 flex gap-2 items-center">
            ...
            <button type="submit" disabled={creating}
              className="bg-red-600 hover:bg-red-500 text-white text-sm font-semibold px-5 py-2 rounded-lg transition disabled:opacity-50">
              {creating ? "Yaratilmoqda..." : "Launch Channel"}
            </button>
          </div>
          {createError && (
            <div className="sm:col-span-3 text-sm text-red-500 bg-red-500/10 border border-red-500/30 rounded-lg px-3 py-2">
              Xatolik: {createError}
            </div>
          )}
        </form>
        ...
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className={cx("rounded-xl border p-5", t.border, t.surface)}>
              ...
              <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
                {schedule.map((slot, i) => (
                  <div key={i} className={cx("flex items-center gap-2 rounded-lg border px-3 py-2", t.border)}>
                    ...
                    {selected.isDb && slot.id && (
                      <>
                        {slot.videoUrl && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />}
                        <label className={cx("p-1 rounded cursor-pointer shrink-0", t.surfaceHover, uploadingId === slot.id ? "text-red-500" : t.textFaint)}>
                          {uploadingId === slot.id ? <span className="text-[10px]">...</span> : <Upload className="w-3.5 h-3.5" />}
                          <input type="file" accept="video/*" className="hidden" disabled={uploadingId === slot.id}
                            onChange={async (e) => {
                              const file = e.target.files?.[0];
                              if (!file) return;
                              setUploadingId(slot.id);
                              try {
                                await uploadScheduleVideo(slot.id, selected.id, file);
                                await onRefreshChannels();
                              } catch (err) {
                                alert("Video yuklashda xatolik: " + (err.message || JSON.stringify(err)));
                              } finally {
                                setUploadingId(null);
                              }
                            }} />
              ...
              {!selected.isDb && (
                <p className={cx("text-[11px] mt-2", t.textFaint)}>Video yuklash faqat siz yaratgan (bazaga ulangan) kanallar uchun ishlaydi.</p>
              )}
              {selected.isDb && (
                <form onSubmit={handleAddSlot} className={cx("flex gap-2 mt-3 pt-3 border-t", t.border)}>
                  <input type="time" required value={newSlotTime} onChange={(e) => setNewSlotTime(e.target.value)}
                    className={cx("text-xs rounded-lg px-2 py-2 border outline-none focus:border-red-500 w-24 shrink-0", t.input)} />
                  <input type="text" required placeholder="Dastur nomi" value={newSlotTitle} onChange={(e) => setNewSlotTitle(e.target.value)}
                    className={cx("flex-1 min-w-0 text-xs rounded-lg px-3 py-2 border outline-none focus:border-red-500", t.input)} />
                  <button type="submit" disabled={addingSlot}
                    className="shrink-0 bg-red-600 hover:bg-red-500 text-white text-xs font-semibold px-3 py-2 rounded-lg transition disabled:opacity-50 flex items-center gap-1">
                    <Plus className="w-3.5 h-3.5" /> {addingSlot ? "..." : "Qo'shish"}
                  </button>
                </form>
              )}
            </div>
...
function AdminPanel({ t, allChannels, removedIds, setRemovedIds, adsState, setAdsState, dbProfiles, isAdmin, onRefreshChannels }) {
  const [deletingId, setDeletingId] = useState(null);
  const realUsers = dbProfiles.map((p) => ({
    id: p.id, name: p.display_name || "Nomsiz", email: p.display_name || "—",
    role: "User", status: "Active", isReal: true,
  }));
  const combinedUsers = [...realUsers, ...ADMIN_USERS.map((u) => ({ ...u, isReal: false }))];
  ...
  async function handleRemoveChannel(c) {
    const removed = removedIds.has(c.id);
    if (c.isDb) {
      if (removed) return;
      if (!isAdmin) { alert("Faqat admin haqiqiy kanalni o'chira oladi."); return; }
      setDeletingId(c.id);
      try {
              ...
              {combinedUsers.map((u) => (
                <tr key={u.id} className={cx("border-t", t.border)}>
                  ...
                  <td className="px-5 py-2.5">
                    ...
                    {!u.isReal && <span className={cx("ml-2 text-[10px]", t.textFaint)}>(demo)</span>}
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              {allChannels.map((c) => {
                ...
                return (
                  <tr key={c.id} className={cx("border-t", t.border)}>
                    <td className={cx("px-5 py-2.5 font-medium", t.text)}>
                      CH {c.chNo} · {c.name} {c.isDb && <span className={cx("ml-1 text-[10px]", t.textFaint)}>(real)</span>}
                    </td>
                    ...
                    <td className="px-5 py-2.5 text-right">
                      <button
                        ...
                        className={cx("text-[11px] font-semibold px-3 py-1 rounded-full border transition disabled:opacity-50",
                        ...
                        {deletingId === c.id ? "..." : removed ? "Restore" : "Remove"}
                      </button>
                    </td>
                  </tr>
                );
...
export default function TeleCastApp() {
  ...
  async function refreshDbChannels() {
    const rows = await fetchDbChannels();
    setDbChannels(rows);
  }
  ...
  useEffect(() => {
    ...
    const mine = dbProfiles.find((p) => p.id === user.id);
    setIsAdmin(!!mine?.is_admin);
  }, [user, dbProfiles]);
  ...
  useEffect(() => {
    if (!user) return;
    fetchUserFollows(user.id).then((ids) => {
  ...
  const allChannels = useMemo(() => {
    ...
    return [...CHANNELS, ...dbChannels, ...created].filter((c) => !removedIds.has(c.id));
  }, [createdChannels, removedIds, dbChannels]);

  const ownedChannels = allChannels.filter((c) => MY_CHANNEL_IDS.includes(c.id) || c.isMine || (user && c.ownerId === user.id));
  ...
  function toggleFollow(id) {
    ...
    if (user) {
      ...
      if (channel && channel.isDb) {
        if (willFollow) dbFollow(user.id, id); else dbUnfollow(user.id, id);
      }
    }
  }
  ...
  return (
    <div className={cx("min-h-screen font-body transition-colors", t.bg, t.text)}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {view === "dashboard" && (
          <OwnerDashboard t={t} isDark={isDark} ownedChannels={ownedChannels} now={now} user={user}
            onOpenAuth={() => setShowAuth(true)}
            onRefreshChannels={refreshDbChannels}
            addChannel={async (c) => {
              if (user) {
                await createDbChannel({ ...c, ownerId: user.id });
                await refreshDbChannels();
              } else {
                setCreatedChannels((prev) => [...prev, c]);
              }
            }} />
        )}
        {view === "admin" && (
          <AdminPanel t={t} allChannels={allChannels} removedIds={removedIds} setRemovedIds={setRemovedIds}
            adsState={adsState} setAdsState={setAdsState}
            dbProfiles={dbProfiles} isAdmin={isAdmin}
            onRefreshChannels={async () => { await refreshDbChannels(); }} />
        )}
      </main>
      ...
      <footer className={cx("border-t mt-12", t.border)}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p className={cx("text-xs", t.textFaint)}>Channels play on their own schedule — just tune in. Demo build with sample data. <span className="opacity-50">(v3)</span></p>
        </div>
      </footer>
    </div>
  );
