...
async function fetchDbChannels() {
  ...
  return channels.map((row, i) => ({
    ...
    schedule: (row.schedule_items && row.schedule_items.length
      ? [...row.schedule_items].sort((a, b) => a.sort_order - b.sort_order).map((s) => ({ time: s.slot_time, title: s.title }))
...
function OwnerDashboard({ t, isDark, ownedChannels, now, addChannel, user, onOpenAuth }) {
